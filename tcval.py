...
class CmafChunksPerFragment(str, Enum):
	...
	MIX: str = 'mix of single and multiple chunks per fragment'
...
class TestContent:
	...
	chunks_per_fragment = ['', '', TestResult.NOT_TESTED]
	...
	def __init__(self, test_stream_id=None, test_file_path=None, mezzanine_version=None, mezzanine_format=None,
		...
		if cmaf_fragment_duration is not None:
			self.cmaf_fragment_duration = [cmaf_fragment_duration, '', TestResult.NOT_TESTED]
		...
		if chunks_per_fragment is not None:
			self.chunks_per_fragment = [chunks_per_fragment, '', TestResult.NOT_TESTED]
		...
		if frame_rate is not None:
			self.frame_rate = [frame_rate, '', TestResult.NOT_TESTED]
		if bitrate is not None:
			self.bitrate = [bitrate, '', TestResult.NOT_TESTED]
		if duration is not None:
			self.duration = [duration, '', TestResult.NOT_TESTED]
...
# Constants
sep = '/'
WAVE_CONTENT_SPEC = "CTA-5001-E" # https://cdn.cta.tech/cta/media/media/resources/standards/pdfs/cta-5001-e-final.pdf
FFMPEG_ERROR_TRACE_HEADERS = "Error initializing bitstream filter: trace_headers"
FFMPEG_ERROR_INVALID_INPUT = "Invalid data found when processing input"
...
def check_and_analyse_ss(ss_test_content, test_content, tc_vectors_folder, frame_rate_family):
	...
	#tc_res_file = open(str(tc_res_filepath), "a", encoding='utf8')
	#json.dump(ss, tc_res_file, indent=4, cls=TestContentFullEncoder, ensure_ascii=False)
	#tc_res_file.write('\n')
	#tc_res_file.close()
...
def analyse_stream(test_content, frame_rate_family, debug_folder):
	...
	print('## Testing '+test_content.test_stream_id)
	
	if CONFORMANCE_TOOL_DOCKER_CONTAINER_ID != '':
		# Run DASH conformance tool
		# Example using local server:
		# docker exec -w /var/www/html/Utils/ 164bd9ff5c45 php Process_cli.php --cmaf --ctawave --segments
		...
		conformance_http_location = 'http://'+local_ip+':'+str(PORT)+str(Path(test_content.test_file_path+sep+TS_MPD_NAME))[len(HTTPD_PATH):].replace('\\', '/')
		...
		json_conformance_tool_output['entries']['SEGMENT_VALIDATION']['MP4BoxValidator']['test'][0]['messages'][0] \
			= json_conformance_tool_output['entries']['SEGMENT_VALIDATION']['MP4BoxValidator']['test'][0]['messages'][0].replace('\r','\n')
	...
	fth_last_lines = collections.deque(''*2, 2)
	for n, line in enumerate(headers_trace):
		if h264_detected:
			...
			if sps_detected and not sps_processed:
				if line.__contains__(' general_tier_flag '):
					file_codec_tier = line.split(' = ')[1][:-1]
					...
					print('Tier = '+h265_tier.get(file_codec_tier, ''))
					continue
			...
			if n > nb_lines-3:
				...
				if test_content.duration[0] == 0:
					...
					test_content.duration[2] = TestResult.PASS \
						if (test_content.duration[0] >= test_content.duration[1] > (test_content.duration[0] - (1 / file_frame_rate))) \
						else TestResult.FAIL
		...
		if not h264_detected and not h265_detected and line.__contains__('Stream #0:0'):
			if line.__contains__('/s,'):
				...
				if test_content.bitrate[0] == TestResult.NOT_APPLICABLE:
					test_content.bitrate[2] = TestResult.NOT_APPLICABLE
				elif test_content.bitrate[0] == 0:
	...
	for m4s in seg_files:
		if m4s.endswith('.m4s'):
			...
			duration_present = \
				...
				or ((sum(file_tfhd_sample_duration_present) + sum(file_trun_sample_duration_present) + sum(file_trune_sample_duration_present)) \
					== len(file_tfhd_sample_duration_present))
			size_present = \
	...
	# Catch untested items
	if test_content.mezzanine_version[2] == TestResult.NOT_TESTED:
		test_content.mezzanine_version[2] = TestResult.FAIL
	if test_content.mezzanine_format[2] == TestResult.NOT_TESTED:
		test_content.mezzanine_format[2] = TestResult.FAIL
	if test_content.conformance_test_result == '':
		test_content.conformance_test_result = {"verdict": "NOT TESTED"}
	if test_content.codec_name[2] == TestResult.NOT_TESTED:
		test_content.codec_name[2] = TestResult.FAIL
	if test_content.codec_profile[2] == TestResult.NOT_TESTED:
		test_content.codec_profile[2] = TestResult.FAIL
	if test_content.codec_level[2] == TestResult.NOT_TESTED:
		test_content.codec_level[2] = TestResult.FAIL
	if test_content.codec_tier[2] == TestResult.NOT_TESTED:
		test_content.codec_tier[2] = TestResult.FAIL
	if test_content.file_brand[2] == TestResult.NOT_TESTED:
		test_content.file_brand[2] = TestResult.FAIL
	if test_content.sample_entry_type[2] == TestResult.NOT_TESTED:
		test_content.sample_entry_type[2] = TestResult.FAIL
	if test_content.parameter_sets_in_cmaf_header_present[2] == TestResult.NOT_TESTED:
		test_content.parameter_sets_in_cmaf_header_present[2] = TestResult.FAIL
	if test_content.parameter_sets_in_band_present[2] == TestResult.NOT_TESTED:
		test_content.parameter_sets_in_band_present[2] = TestResult.FAIL
	if test_content.picture_timing_sei_present[2] == TestResult.NOT_TESTED:
		test_content.picture_timing_sei_present[2] = TestResult.FAIL
	if test_content.vui_timing_present[2] == TestResult.NOT_TESTED:
		test_content.vui_timing_present[2] = TestResult.FAIL
	if test_content.vui_primaries_mcoeffs[2] == TestResult.NOT_TESTED:
		test_content.vui_primaries_mcoeffs[2] = TestResult.FAIL
	if test_content.vui_transfer_characteristics[2] == TestResult.NOT_TESTED:
		test_content.vui_transfer_characteristics[2] = TestResult.FAIL
	if test_content.sei_pref_transfer_characteristics[2] == TestResult.NOT_TESTED:
		if test_content.sei_pref_transfer_characteristics[0] == '':
			test_content.sei_pref_transfer_characteristics[2] = TestResult.NOT_APPLICABLE
		else:
			test_content.sei_pref_transfer_characteristics[2] = TestResult.FAIL
	if test_content.sei_content_light_level[2] == TestResult.NOT_TESTED:
		if test_content.vui_transfer_characteristics[0] != transfer_characteristics_values.get("PQ10"):
			test_content.sei_content_light_level[2] = TestResult.NOT_APPLICABLE
		else:
			test_content.sei_pref_transfer_characteristics[2] = TestResult.FAIL
	if test_content.sei_mastering_display_colour_vol[2] == TestResult.NOT_TESTED:
		if test_content.vui_transfer_characteristics[0] != transfer_characteristics_values.get("PQ10"):
			test_content.sei_mastering_display_colour_vol [2] = TestResult.NOT_APPLICABLE
		else:
			test_content.sei_pref_transfer_characteristics[2] = TestResult.FAIL
	if test_content.cmaf_fragment_duration[2] == TestResult.NOT_TESTED:
		test_content.cmaf_fragment_duration[2] = TestResult.FAIL
	if test_content.cmaf_initialisation_constraints[2] == TestResult.NOT_TESTED:
		test_content.cmaf_initialisation_constraints[2] = TestResult.FAIL
	if test_content.chunks_per_fragment[2] == TestResult.NOT_TESTED:
		test_content.chunks_per_fragment[2] = TestResult.FAIL
	if test_content.b_frames_present[2] == TestResult.NOT_TESTED:
		test_content.b_frames_present[2] = TestResult.FAIL
	if test_content.cmf2_sample_flags_present[2] == TestResult.NOT_TESTED:
		test_content.cmf2_sample_flags_present[2] = TestResult.FAIL
	if test_content.resolution[2] == TestResult.NOT_TESTED:
		test_content.resolution[2] = TestResult.FAIL
	if test_content.pixel_aspect_ratio[2] == TestResult.NOT_TESTED:
		test_content.pixel_aspect_ratio[2] = TestResult.FAIL
	if test_content.frame_rate[2] == TestResult.NOT_TESTED:
		test_content.frame_rate[2] = TestResult.FAIL
	if test_content.bitrate[2] == TestResult.NOT_TESTED:
		test_content.bitrate[2] = TestResult.FAIL
	if test_content.duration[2] == TestResult.NOT_TESTED:
		test_content.duration[2] = TestResult.FAIL
	if test_content.mpd_sample_duration_delta[2] == TestResult.NOT_TESTED:
		test_content.mpd_sample_duration_delta[2] = TestResult.FAIL
